...
class Library():
    ...
    def __init__(self, name, floornames, occupied_places, avg_temp, avg_hum, max_places = 100, opening = 9, closing = 20):
        #temp, hum, entrance, s0, s1, s2, s3, progr
        self.name = name
        self.floornames = floornames
        self.output = [0,0,0,0,0,0,0,0]
        self.max_places = max_places
        self.places = max_places - occupied_places
        ...
        for i in range(len(self.floornames)):
            self.floors[self.floornames[i]]= Room(self.floornames[i], self.places, self.max_places, self.avg_temp, self.avg_hum)
    ...
    def entrance(self, h, m):
        ...
        if self.places < self.max_places and self.places > 0:
        ...
        elif self.places == self.max_places:
            if abs(r) > 0.35:
                entrance = -1
    ...
    def generate_values(self, h, m):
        ...
        print(self.name, "Posti disponibili: ", self.max_places - self.places)
